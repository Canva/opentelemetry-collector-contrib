...
func TestNewTracesExporter(t *testing.T) {
	for _, tt := range []struct {
		...
		},
		{
			...
			resourceKeysRouting,
			[]string{string(conventions.ServiceNameKey)},
			nil,
		},
		{
			"resource_keys",
		...
		t.Run(tt.desc, func(t *testing.T) {
			// test
			te, err := newTracesExporter(exportertest.NewNopSettings(metadata.Type), tt.config)
...
func TestAttributeBasedRouting(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			res, err := routingIdentifiersFromTraces(tc.batch, attrRouting, tc.attributes, nil)
...
func TestUnsupportedRoutingKeyInRouting(t *testing.T) {
	...
	_, err := routingIdentifiersFromTraces(traces, 38, []string{}, nil)
...
func TestServiceBasedRoutingForSameTraceId(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			res, err := routingIdentifiersFromTraces(tt.batch, tt.routingKey, tt.attrs, tt.resKeys)
			assert.NoError(t, err)
...
func TestResourceKeysBasedRoutingIdentifiers(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			res, err := routingIdentifiersFromTraces(tt.batch, resourceKeysRouting, nil, tt.resKeys)
			assert.NoError(t, err)
			assert.Equal(t, res, tt.res)
		})
	}
...
func TestNoTracesInBatch(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			res, err := routingIdentifiersFromTraces(tt.batch, tt.routingKey, tt.routingAttrs, nil)
